...
function switchTab(event, tabId) {
    ...
    if (event) {
        ...
        const targetBtn = Array.from(tabButtons).find(btn => btn.getAttribute("onclick").includes(tabId));
...
function updatePipelineNode(activeNodeId) {
    const nodesOrder = ["node-start", "node-search", "node-ingest", "node-select", "node-analyze", "node-report", "node-finish"];
    ...
    nodesOrder.forEach((nodeId, idx) => {
        ...
        const lineEl = nodeEl.nextElementSibling && nodeEl.nextElementSibling.classList.contains("pipeline-line") 
...
async function startResearch() {
    ...
    // Clear tabs details
    document.getElementById("state-plan-content").innerText = "Chờ kế hoạch nghiên cứu...";
    document.getElementById("state-findings-content").innerText = "Chờ kết quả phân tích...";
    document.getElementById("state-comparison-content").innerText = "Chờ bảng so sánh...";
    document.getElementById("state-trends-content").innerText = "Chờ phân tích xu hướng...";
...
function handleSSEEvent(event, data) {
    switch (event) {
        ...
        case "tool_call":
            appendLog("tool", `Chạy tool [${data.tool}] với tham số: ${JSON.stringify(data.arguments)}`);
        ...
        case "complete":
            appendLog("complete", "Quá trình nghiên cứu hoàn tất! Báo cáo đã sẵn sàng.");
        ...
        case "error":
            appendLog("error", `LỖI TẠI NODE [${data.node || "Hệ thống"}]: ${data.message}`);
            if (data.traceback) {
...
function mapToolToPipeline(toolName) {
    const toolSearch = ["search_website_url", "fetch_web_content", "arxiv_search"];
...
function handleStateUpdate(key, value, node) {
    ...
    if (key === "plan") {
        ...
        document.getElementById("state-findings-content").innerHTML = findingsHtml || "Chưa có findings trích xuất.";
    }
